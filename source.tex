...
\begin{prop}[Density of the GP]\
  ...
  \begin{proof}
    ...
    Please refer to \cite{10909705}, for thorough explanation of the interpretation of $\kappa$.
  \end{proof}
\end{prop}
...
The partial densities can be derived from the density of the $n$th factorial moment measure as \cite[Lisää yhtälö]{7305791}
...
The joint pdf of the $k$ strongest values of the STIR process $(\mathsf{Z}'_{(1)}, \dots, \mathsf{Z}'_{(n)})$ is given as a series expansion involving the partial densities \cite[Lisää yhtälö]{7305791}
\begin{equation}
...
with $\theta'=\theta/(1+\theta)$ and $i_{\text{max}}<1/\theta'-k$.
    ...
    The density of the $n$th factorial moment measure of the SIR process can be extracted from $\mu'^{(n)}$ \cite[Lisää yhtälö]{7305791}:
    \begin{align}
      \label{eq:densitySINR}
      &\mu^{(n)}(z_1,\dots,z_n)&  \nonumber\\
      &= \prod_{j=1}^n\frac{1}{(1+z_j)^2}\mu'^{(n)}\left(\frac{z_1}{1+z_1},\dots,\frac{z_n}{1+z_n}\right)
