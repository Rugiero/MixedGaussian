...
Considering a uplink, the UEs follow a homogeneous PPP $\Phi \subset \R^2$ of density $\lambda$. The BSs form an independent homogeneous p.p. possibly with different density than the UEs. Because of the translation invariancy of the PPP, all location are statistically equivalent, and we consider the typical BS to be located at the typical point $\textit{o}$.
...
The path loss is given over the planar distance $r \in [0, \infty)$ as a Gaussian function:
  ...
  \end{equation}
  The angle $\varphi_{\text{RX}}$ denotes the $-3$ dB antenna gain width. Furthermore,, the scaling constant $D_{h,\epsilon}$ multiplied by the distance $r$ to the closest focus pont of the elliptical fooptrint; $D_{h,\epsilon}r$, approximates the angle of a UE in the (narrow-beam) LEO BS Gaussian antenna gain. %% This is a sufficient approximation for small $r$, for example, for UEs within the main lobe in case of narrow beams characterozed by small $\varphi_{\text{RX}}^2$. Otherwise, because of the fast decay of the Gaussian path loss, the power from the UES with large $r$ practically vanishes.

  At the typical location, $r=\|x\|$, where $\| x\|$ is the Euclidean distance between the typical BS at \textit{o} and the UE $x\in \Phi$.


  \begin{remark}[Terrestrial millimeter communication]
    The Gaussian, \textit{i.e.}, the squared exponential spatial path loss function have applications in terrestrial networks that pose rapid signal attenuation, such as millimeter wave cellular networks in dense urban environments. Hence, the analysis presented in the paper can be applied to such planar Poisson network models. Notably, the power term $r^2$ is particularly tractable for the second power---as is the case in Gaussian function.
  \end{remark}
...
\end{equation}
There is no requirement for the symbols representing the points in the point process (p.p.) to differ on the l.h.s. and r.h.s. summation notations. However, we use $u \in \mathcal{G}$ to represent the signal strength at $\textit{o}$ at the BS to clearly separate it from $x$, which represents a spatial location.
