...
\begin{equation}
  \label{eq:totpow}
  I \triangleq \sum\limits_{x \in \Phi} H_x G(D_{h,\epsilon}\|x\|)=\sum\limits_{x \in \mathcal{G}}x.
\end{equation}
...
Let us consider that the strongest signal is decoded directly if its SINR exceeds $\theta$; otherwise, signal cancellation is attempted. We denote the SINR with independent interference cancellation as \text{IIC-SINR}$_k$, where interference from signals with indices in the set $[k] \triangleq \{1, \dots, k\}$ is canceled if each $(\mathsf{Z}_{(1)}, \dots  \mathsf{Z}_{(k)})$ reaches the signal separation threshold $\tau$. The coverage probability is defined by
...
Considering the natural base of the log-normal distribution, the constant $\rho \triangleq \log(10)/10$ normalizes the parameters $\mu_{\text{LoS}}, \sigma_{\text{LoS}}, \mu_{\text{NLoS}},$ and $\sigma_{\text{NLoS}}$ so that $10 \log_{10}(H_{\mathcal{M}\mathcal{L} \mathcal{N}})$ evaluates to a mixed normal r.v. $p_{\text{LoS}} \mathcal{N}(\mu_{\text{LoS}}, \sigma_{\text{LoS}}^2) + p_{\text{NLoS}} \mathcal{N}(\mu_{\text{NLoS}}, \sigma^2_{\text{NLoS}})$.


 Furthermore, we introduce a constant scaling term, $\Upsilon_{\epsilon}$, ensuring that the means of the mixed log-normal distribution and the defected exponential distributions match; $\mathbb{E}(\Upsilon^{}_{\epsilon}H_{\mathcal{M}\mathcal{L}\mathcal{N}})= \mathbb{E}(H_{\text{Exp}}) = \upsilon_{\epsilon}$.  Namely, by equating the first two moments of $H_{\text{Exp}}$ (l.h.s.) and $\Upsilon_{\epsilon} H_{\mathcal{M} \mathcal{L}\mathcal{N}}$ (r.h.s.),
\begin{equation}
  ...
  \end{cases}
\end{equation}
to match the moments, we can solve for the parameters
\begin{align}
...
\end{align}
Recall equations \eqref{eq:totmean} and \eqref{eq:totvar}. Matching the first two moments of fading distributions is equivalent to matching the mean and variance of total interferences using the respective fadings. 


Without loss of generality, we can set $\Upsilon_{\epsilon}=1$  when studying the SIR scenarios, as the scaling of the signal powers cancel for $N=0$. In the SINR case, the scaling is accounted for by adjusting the noise constant by a factor of $N \sim 1/\Upsilon_{\epsilon}$. The parameter $0 \leq  1-\upsilon_{\epsilon} < 1$ of the defective exponential distribution (somewhat vaguely) encapsulates the shadowing occurrences, when the signal is blocked almost entirely  to $-\infty$ dB in the mixed log-normal shadowing model. These occurrences are particularly common in low elevation-angles, because of tall buildings and other similar large objects preventing the LoS and blocking the signal.

The UEs with the strongest signals are unlikely to be in a significant blockage, and definitely the strongest signals are never blocked entirely (or destructively interfered!!) (literally by meaning ``entirely'' in the mixed exponential shadowing when the power maps to $0$, and ``almost entirely'' in the mixed log-normal shadowing, when the power is mapped close to $0$). On the other hand, the interfering UEs of the smaller order statistics have larger probability to be blocked by large obstacles and hence exhibiting NLoS behavior and severe shadowing. The portion of signals that are blocked entirely, is reflected, in the densities of the $n$th factorial moment measures \eqref{eq:factorialmoment} and \eqref{eq:nthmomentmeasureSTINR}, where the mean number of UEs inside the $3$ dB footprint of a SBS; ${\kappa}=\tilde{\kappa}\log(2)  $, is scaled by the factor $0 <\upsilon_{\epsilon} \leq 1$; $\upsilon_{\epsilon} \kappa=\upsilon_{\epsilon}\tilde{\kappa} \log(2) $: $\upsilon_{\epsilon}$ corresponding to the fraction of \textit{effective} UEs having a non-zero shadowed signal power at the typical SBS.

WHAHATT HURR;;;
Being general, and including insight, we use the effective number of UEs $\upsilon_{\epsilon}\tilde{\kappa} \log(2)$ in the plots instead of the actual number of UES $\upsilon_{\epsilon}\tilde{\kappa} \log(2)$. The simulations are also scaled accordingly.


%% Note that we considerer mobile hand-held UEs located in an urban environment, directly contacting the SBS. However, implementing a hybrid network, so that each UE would be linked to the SBSs though a terrestrial fixed BS that could be positioned to a high location with clear view to the SBS, improving the shadowed signal power of the served signal, potentially improving the performance of the link (on the other hand, the total blockages also reduce the interference, which is beneficial for the coverage probability). In this work, we omit such hybrid networks.



\subsection{Planar model of the narrow-beam LEO uplink and the constant spatial path loss}

As indicated in the analysis in Section \ref{sec:analysissec}, we incorporate a flat-earth model. The planar model eqffectively approximates the spherical model \ref{eq:sphericalmodel} with narrow beams. The UEs, \textit{i.e.}, the Earth transmitters, radiate omnidirectionally and form a homogeneous PPP $\Phi \subset \R^2$, Gaussian path loss $G(D_{h,\epsilon}\|x\|)$ representing the signal gains at the SBS, and i.i.d. shadowing $H_x$. The SBS antenna boresight is directed at the origin $\textit{o} \triangleq (0,0)$. Please refer to Figure \ref{fig:UEsontheplane} for the geometric interpretation.

The scaling term $D_{h,\epsilon} = \sin^2(\epsilon)/h$ is essentially the first-order coefficient in the Taylor expansion of the function $\|x\| \mapsto \varphi_x$. It is derived in Appendix \ref{app:constantD}.
...
The distance $\hat{d}_{h,\epsilon} \triangleq h/\sin(\epsilon)$ can be derived through simple geometry.

Because $\Upsilon_{\epsilon} u/\ell(\hat{d}_{h,\epsilon})/(\Upsilon_{\epsilon}  I/\ell(\hat{d}_{h,\epsilon})+W) > \theta $ and $u /(I+W \ell(\hat{d}_{h,\epsilon})/\Upsilon_{\epsilon})> \theta, u \in \mathcal{G},$ are equivalent conditions (for some $W>0$), the spatial path loss is simply incorporated into the noise term $N$ (as introduced \eqref{eq:SINR} and \eqref{eq:STINR}) along side with with the scaling term $\Upsilon_{\epsilon}$. Clearly, the constants $\ell(\hat{d}_{h,\epsilon})$ and $\Upsilon_{\epsilon}$ are only relevant in the interference-plus-noise channel, canceling out if $N=0$. 


Namely, taking into account spatial path loss and the scaling of signal powers, the noise term in \eqref{eq:SINR} and \eqref{eq:STINR} is given by:
\begin{equation}
N \triangleq \frac{W \ell(\hat{d}_{h\epsilon})}{\Upsilon_{\epsilon}},
\end{equation}
where $W \geq 0$ is a dimensionless noise power constant at the SBS. As noted, is this work, the only instance for this $N$ to be accounted for is when evaluating the $n$th factorial moment measure of the STINR process; $\eqref{eq:nthmomentmeasureSTINR}$.


\begin{figure}[h]%
  \centering
  \subfloat[\centering  Interpretation of the \textit{planar} system model with the SBSs in adjacent orbits serving an urban area and a realization of the UEs. The altitudes are not to scale.]{{\includegraphics[width=\linewidth]{UEsontheplane.png}}
    \label{fig:UEsontheplane}%%
  } 
  \qquad
  \subfloat[\centering The typical SBS as seen from the side. The transmitters are projected into line $(0, \infty)$ according to their norm.]{{\includegraphics[width=\linewidth]{drawing.png}}     \label{fig:systemmodel}} %%
...
\subsection{Spherical model of the narrow-beam LEO uplink}
\label{eq:sphericalmodel}
The base line system model is the spherical model, where the UE are located on the spherical Earths surface according to the PPP $\Theta$ on the Earth surface. Like in the planar model, the UEs are radiating omnidirectionally and exhibits random shadowing.

The total received power at the typical SBS from the UEs in the PPP $\Theta$ on the Earth's surface above the horizon is defined as

\begin{equation}
  \label{eq:mathringptot}
  \mathring{I} \triangleq (\mathring{I}- \mathring{S}) + \mathring{S} =    \sum_{u \in \Theta} H_u\ell(u) = \sum_{u \in \Theta}  \frac{H_u G_{u}(\varphi_u)}{(d_u/d_0)^{\alpha}},
\end{equation}
$H_{u} \sim H_{\mathcal{M}\mathcal{L}\mathcal{N}}$ are i.i.d. following the mixed log-normal distribution inroduced in Section \ref{sex:guassianmixture}. The power path lossexponent $\alpha \geq 0$, and $d_0$ is a normalizing distance accounting for any other factors affecting the path loss, such as the frequency band of the signals, and casts the path loss to a dimensionless quantity. Where $\varphi_u$ is the angle between the SBS antenna boresight and  $u \in \Theta$. Each $H_{\mathcal{M}\mathcal{L}\mathcal{N}}$ is a i.i.d. shadowing variables. Defined implicitly in \eqref{eq:mathringptot}, $\mathring{S}$ signal power from the strongest signal and $\mathring{I}-\mathring{S}$ is the total power from the other signals. The SIR and SINR $\mathring{S}/(\mathring{I}-\mathring{S})$,  $\mathring{S}/(\mathring{I}-\mathring{S}+N)$, are defined accordingly. The IC schemes SIR-IC, SIR-SIC, SINR-IC and SINR-SIC are simulated using these definitions. 


The spherical model is analytically needlessly complex to work with. For the narrow-beamed antenna gain, such as the Gaussian antenna beam, the planar model introduced in the following section stricingly well approximates the spherical model, especially for the large elevation angles or small SBS altitudes. However, in this work, we exclude the implementation details of the spherical model and refer to [???] for an extensive comparison between the planar model and the spherical model in the narrow beam LEO upink. Also, the presented numerical results are always compared to the spherical model.
