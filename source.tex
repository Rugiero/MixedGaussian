...
\end{align}
with the upper summation limit bounded by $i_{\text{max}} < 1/\tau'-1=1/\tau.$ Note the upper bound $\theta'$ of the integral, through which we omit the probability mass of the even $\mathsf{Z}_{(1)}>\theta$. The SINR-IIC$_{(k)}$ case $\Delta^{(k)}_{\text{IIC}}(\theta,\tau)$ can be obtained by dropping the summation term inside the left-hand side identity function in $\eqref{eq:SICprob}$ (which only essentially lower bounds the integration domain by $\tau'$).
