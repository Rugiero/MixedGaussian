...
In this paper, we study the SIR under the SIC of user equipments (UEs) at a typical LEO satellite base station (LEO BS) in an urban shadowing environment. We utilize the Gaussian Mixture shadowing model, similar to \cite{modelinguplink} and \cite{9684552}. We demonstrate that the link quality can be made more stable with interference cancellation, while maintaining good performance, through increasing the network density while mitigating interference with the SIC, which enables serving multiple UEs at each LEO BS.
...
In the paper, a highly simplified moment-order-two approximation for log-normal shadowing is proposed and shown to be feasible: a \textit{mixture} exponential distribution with a delta at $0$ (\textit{two} degrees of freedom instead of \textit{one}). The exponential fading distribution typically yields the simplest and most insightful results in stochastic geometry analysis, which can be leveraged by this approximation. Furthermore, for the analysis, we propose a planar system, which is feasible for the narrow-beamed LEO BS networks. The analytical results are verified by comparing them to Monte Carlo simulated performance metrics acquired by the realistic system model with a spherical Earth.
...
As a compromise between analytical tractability and realism, we introduce an exponential distribution with an atomic probability measure at $0$, described by the complementary CDF (CCDF)
\begin{equation}
  \label{eq:defexp}
...
Note that $\kappa \upsilon = 3$ is in the finite-variance region regarding the SIR distribution of each transmitter (recall, \eqref{eq:SIR2}): the link quality is more stable at the LEO BS compared to a smaller UE density scenario, say, $\kappa \upsilon =1$, when the SIR variance is infinite. This is desirable if we want a consistent user experience. Explicitly, a (loose) upper bound for the variance of the strongest UE in Figures \ref{fig:nprobability} and \ref{fig:ASICSIR} can be calculated in accordance with \eqref{eq:SIR1} and \eqref{eq:SIR2}: $\text{var}(\text{SIR}_{(1)}) =\mathbb{E}(\text{SIR}^2_{(1)})-\mathbb{E}(\text{SIR}_{(1)})^2  < 1.3$.  In this regard, the SIC-SIR enables more consistent link quality while allowing good performance also for the second and third strongest UEs. Let us study the total throughput next.



Considering the SIR distributions in Figure \ref{fig:ASICSIR}, using the trapezoidal method, we can calculate the throughput \eqref{eq:throughput} as $\mathcal{T}_{3}\approx 0.7 \textup{ bit/s/Hz}$. On the other hand, in the infinite-variance SIR region with one served UE in each cell on average ($\kappa \upsilon =n=1$), we have for the served transmitter $\mathcal{T}_1 \approx 1$ bit/s/Hz: Under the SIC with $\kappa \upsilon=n=3$, there is a considerable but not overwhelming $-0.3$ bit/s/Hz tradeoff for link stability (and number of served UEs) as the cost of average network throughput. %It is worth noting that the \textit{individual} UE throughputs (contrary to the \textit{total} throughputs of each served UE at the LEO BSs) are, on average, more significantly reduced with higher $\kappa \upsilon$.

This work presents a comprehensive stochastic geometry framework for performance analysis of the typical LEO BS under interference cancellation in the uplink. The model translates to downlink, considering that the cell locations are located according to the PPP on the Earth. Furthermore, the framework extends straightforwardly to signal combination. A clear improvement in the SIR of the second and third strongest UEs was observed under the SIC. The SIC can also be utilized to attain a consistent link quality while maintaining a good average throughput over the LEO BSs.
