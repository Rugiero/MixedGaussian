...
Let $\{u_1,\dots,u_k \}\subset \mathcal{G}$ be an ordered set of points in the GP, $u_1$ denoting the strongest signal at the SBS. Consider that the strongest $k$ signals are canceled from the interference. We define the SINR of the strongest signal under interference cancellation (IC-SINR) as
\begin{equation}
...
\begin{align}
  ...
  &= \mathbb{P} \left\{ \mathsf{Z}'_{(n)}+\theta'\sum_{j\neq n}\mathsf{Z}'_{(j)} +>\theta'\right\}.
\end{align}
...
\end{align}
where the $1$-coverage probability  $\mathcal{P}^{(1)}(\theta)$ is given in $\eqref{eq:kprobability}$. Using the joint probability density function of the STIR process \eqref{eq:jointprobability}, we have for the SIC-SINR
\begin{align}
  \label{eq:SICprob}
...
\end{align}
with the upper summation limit bounded by $i_{\text{max}} < 1/\tau'-1=1/\tau.$ Note the upper bound $\theta'$ of the integral, through which we omit the probability mass of the even $\mathsf{Z}_{(1)}>\theta$. The SINR-IIC$_{(k)}$ case $\Delta^{(k)}_{\text{IIC}}(\theta,\tau)$ can be obtained by dropping the summation term inside the left-hand side indicator function in $\eqref{eq:SICprob}$ (which only essentially lower bounds the integration domain by $\tau'$).
...
in the   $\rho_{\epsilon}\tilde{\kappa}=2.6$, $\mathbb{E}(\text{SIC-SINR}_{1,3}) = \int_{0}^1 \mathcal{P}^{(3)}_{\text{SIC}}(y,\tau) dy \approx 2.4$. For $\rho_{\epsilon}\tilde{\kappa}=3.8$, $\mathbb{E}(\text{SIC-SINR}_{1,3}) = \int_{0}^1 \mathcal{P}^{(3)}_{\text{SIC}}(y,\tau) dy \approx 2.4$.
