...
\begin{prop}[SIC-SIR]
  ...
  \begin{align}
    \label{eq:SICprob}
     &\mathcal{P}^{(n)}_{\textup{SIC}}(\theta)\triangleq \int_{0}^{1} \dots \int_{0}^{1}d z'_1 \dots  d z'_n \times f'_{(n)}(z'_1,\dots,z'_n)      \nonumber \\
    &  \times \mathds{1}\left( z'_m+ \theta'\sum_{j=1}^{m-1} z'_j>\theta'  \right)\prod_{m=1}^n \mathds{1}\left( z'_m+ \tau'\sum_{j=1}^{m-2} z'_j>\tau'  \right)
  \end{align}
  with the upper summation limit in $f'_{(n)}(\cdot)$ bounded by $i_{\text{max}} < 1/\tau'-1=1/\tau$.
  \begin{proof}
...
The performance of the $2$nd and $3$rd UEs is significantly enhanced with the SIC-SIR compared to the $n$-probability, especially for small $\theta$ (\textit{i.e.}, low-rate/throughput transmissions), showing a $((0.8180-0.0870)/0.0870) \times 100 \% \approx 840 \%$ improvement for the coverage probability of the third UE with $\theta = -7$ dB. 
...
Note that $\kappa \upsilon = 3$ is in the finite-variance region regarding the SIR distribution of each transmitter (recall, \eqref{eq:SIR2}): the link quality is more stable at the SBS compared to a smaller UE density scenario, say, $\kappa \upsilon \in (\log(2),2\log(2)]$, when the SIR variance is infinite. This is desirable if we want a consistent user experience. Explicitly, an (loose) upper bound for the variance of the strongest UE in Figures \ref{fig:nprobability} and \ref{fig:ASICSIR} can be calculated using \eqref{eq:SIR1} and \eqref{eq:SIR2}: $\text{var}(\text{SIR}_{(1)}) =\mathbb{E}(\text{SIR}^2_{1)})-\mathbb{E}(\text{SIR}_{(1)})^2  < 1.3$.  In this regard, the SIC-SIR enables more consistent link quality while maintaining a reasonable throughput at the SBSs. 
 ...
 Considering an adaptive encoding and decoding that adapts according to the SIR of the served UE (after the SIC) at the typical SBS, the expected total bandwidth-normalized Shannon throughput \cite[Eq. (7.19)]{alma998193414406526} of the three served UEs is (case Figure \ref{fig:ASICSIR})
\begin{align}
  &\mathcal{T}=\sum_{n=1}^3 \mathbb{E}\log\left(1+\text{SIR}^{(n)}_{\text{SIC}}\right)/\log(2) \nonumber \\
  &=\int_{0}^{\infty}\frac{\mathcal{P}^{(1)}_{\textup{SIC}}(t)+\mathcal{P}^{(2)}_{\textup{SIC}}(t)+\mathcal{P}^{(3)}_{\textup{SIC}}(t)}{(t+1)\log(2)}d t  \approx 0.7 \textup{ bit/s/Hz}.
\end{align}
