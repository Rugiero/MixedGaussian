...
Let us consider that the strongest signal is decoded directly if its SINR exceeds $\theta$; otherwise, signal cancellation is attempted. Accordingly, we define a IIC-SINR coverage probability of the strongest signal by
...
\end{equation}
Similarly, the SIC-SINR coverage probability is defined by
\begin{equation}
...
As a metric of the performance improvement, we study the difference between the coverage probability without any interference cancellation and the IIC or SIC. Define the differences
\begin{align}
...
For the SIC-SINR, the performance increase is given by
\begin{align}
  \label{eq:SICprob}
...
\end{align}
with the upper summation limit bounded by $i_{\text{max}} < 1/\tau'-1=1/\tau.$ The IIC case $\Delta^{(k)}_{\text{IIC}}(\theta,\tau)$ can be obtained by dropping the summation term inside the left-hand side identity function in $\eqref{eq:SICprob}$ (which essentially lower bounds the integration domain by $\tau'$).

We observed that for $\tau = -7$ dB, the best results for SIC is obtained for $k=4$, whereas in IIC for $k=2$.?!?!
