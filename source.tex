...
Recall equations \eqref{eq:totmean} and \eqref{eq:totvar}. Matching the first first two moments of the fading distributions $H=H_{\text{Exp}}$ and $H=H_{\mathcal{M}\mathcal{L}\mathcal{G}}$ is equivalent to matching the mean and variance of the corresponding total interferences (before the spatial path loss) $I = \sum_{x \in \mathcal{G}} H_x x$.
 ...
 Note that without loss of generality, we can set $\Upsilon_{\epsilon}=1$  when studying the SIR, as the scaling of the signal powers cancel.


\subsection{Spherical model of the narrow-beam LEO uplink}
\label{eq:sphericalmodel}
The base line system model is the spherical model, where the UE are located on the spherical Earths surface according to the PPP $\Theta$ on the Earth surface. The UEs are radiating omnidirectionally.

We define the derved UE in the spherical model as ${u}_0 \triangleq \arg \min \{ u \in \Theta: d_u\}$. The total received power at the typical SBS from the UEs in the PPP $\Theta$ on the Earth's surface above the horizon is defined as

\begin{equation}
  \label{eq:mathringptot}
  \mathring{I} \triangleq \mathring{I} + \mathring{S} =    \sum_{u \in \Theta} H_u\ell(u) = \sum_{u \in \Theta}  \frac{H_u G_{u}(\varphi_u)}{(d_u/d_0)^{\alpha}},
\end{equation}
$H_{u} \sim H_{\mathcal{M}\mathcal{L}\mathcal{N}}$ are i.i.d. following the mixed log-normal distribution inroduced in Section \ref{sex:guassianmixture}. The path loss powe rexponent $\alpha \geq 0$, and $d_0$ is a normalizing distance accounting for any other factors affecting the path loss, such as the frequency band of the signals, and casts the path loss to a dimensionless quantity. Where $\varphi_u$ is the angle between the SBS antenna boresight and  $u \in \Theta$. Each $H_{\mathcal{M}\mathcal{L}\mathcal{N}}$ is a i.i.d. shadowing variables. The SIR and SINR $\mathring{S}/\mathring{I}$,  $\mathring{S}/(\mathring{I}+N)$, are defined accordingly. 


The spherical model succesfully approximates the planar model, and the reader may refer to [???] for a extensive comparison between the planar model and the spherical model in the narrow beam LEO upink. We omit the geometrical details or the implementation details of the spherical model.
