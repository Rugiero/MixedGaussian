...
\maketitle
\begin{abstract}
  We investigate the factorial moment measure of the signal-to-interference ratios (SIR) at the typical low Earth orbit satellite base station (SBS) with a narrow Gaussian antenna serving an urban area, with a Gaussian mixture shadowing model. The signal-to-total-interference (STIR) process is characterized by a Poisson-Dirichlet distribution $\text{PD}(0, \cdot)$, which allows us to derive the density of the factorial moment measure. We analyze the coverage probability at the typical SBS receiving the three strongest signals and implement successive interference cancellation (SIC). The results indicate that by SIC, we can, \textit{e.g.}, make the network twice as dense while preserving the performance of the strongest user equipment (UE).
\end{abstract}
...
Successive interference cancellation (SIC) is used in non-orthogonal multiple access (NOMA) systems. We study the SIR of user equipments (UEs) at the typical LEO satellite base station (SBS) by utilizing the narrow-beam LEO uplink system model from \cite{10909705} in an urban environment. We utilize the Gaussian Mixture shadowing model, similar to \cite{modelinguplink} and \cite{9684552}, using the parameters presented in \cite{TR38.811}. The density of the factorial moment measure of the signal-to-total-interference (STIR) process follows a Poisson-Dirichlet distribution PD$(0,\cdot)$ (contrary to PD$(\cdot,0)$ in \cite{7305791}), of which the factorial moment measure is well-known. We derive the joint PDF of the STIR and SIR and study the performance metrics of the three strongest UE signals with and without SIC. In \cite{10909705}, it was observed that the system parameters optimizing the average throughput, corresponding to mean $\log(2)$ user equipments (UEs) inside a satellite base station SBS $-3$ dB footprint, lead to a high variation in the SIR over the SBSs. In this paper, we demonstrate that the link is more stable with interference cancellation because we can increase the network (and constellation density) and control the interference with SIC. Furthermore, the coverage probability of UEs with weaker signals drastically improves. 

A SIC scheme was studied in \cite{11030605} in the stochastic geometry context. However, while the order statistics of the signal-to-interference ratio (SIR) under interference cancellation (and signal combination) have been studied for terrestrial networks \cite{7305791} using stochastic geometry, they have yet to be explored in low Earth orbit (LEO) networks.
...
A narrow-band LEO  uplink is considered. The UEs follow a homogeneous PPP $\Phi \subset \R^2$ of density $\lambda$. The SBSs form a homogeneous point process (p.p.) possibly with a different density than the UEs. Because of the translation invariancy of the PPP, all locations are statistically equivalent, and we define the origin $\textit{o}$ to represent \textit{the typical SBS} footprint focus point.
...
We conclude that successive interference cancellation (SIC) is a viable solution for mitigating the considerable variability in link quality over the satellite base stations (SBSs) in a narrow-beam LEO uplink and improving the SIR of the second and third strongest transmitters at each SBS.
