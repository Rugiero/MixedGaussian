...
Although our study ultimately focuses on a narrow-beam LEO uplink, we initially formulate our analysis using a traditional stochastic geometry model of wireless networks in the plane. This model incorporates a (defective) exponential fading distribution---wherein the terms ``fading'' and ``shadowing'' are used interchangeably in this work---and Gaussian path loss. We interpret this model as representative of a LEO scenario with Gaussian mixture shadowing in Sections \ref{sec:guassianmixture}, \ref{sec:planarmodel}, and \ref{sec:sphericalmodel}. Additionally, we present numerical comparisons between the analytical results and Monte Carlo simulations using a spherical Earth model with Gaussian mixture shadowing.


We assume a homogeneous Poisson point process (PPP) $\Phi \subset \mathbb{R}^2$ with density $\lambda$. Due to the translation invariance of the PPP, we can refer to the \textit{typical point}, which statistically represents any point on the plane and is defined as the origin $\textit{o} \triangleq (0,0)$. The typical point represents a base station (BS) in an uplink scenario. However, the points in the PPP could also represent the BSs, transforming the model into a downlink scenario. We evaluate the SIR and SINR at the typical location for the strongest $k \geq 1$ shadowed signals from the transmitters in the PPP. Additionally, we introduce the concept of interference cancellation (IC) in the forms of independent interference cancellation (IIC) and successive interference cancellation (SIC). These methods prove useful in optimizing performance and reducing variance in link quality among the BSs.



\subsection{Gain process (path loss process)}
In this section, we only need to characterize the path loss resulting from the Gaussian antenna gain patterns of the satellite BSs. It is given over the planar distance $r \in [0, \infty)$ as
  \begin{equation}
    \label{eq:Gaussianantpat}
  G(r) = 2^{-(D_{h,\epsilon}r)^2/\varphi_{\text{RX}}^2},
\end{equation}
where $\varphi_{\text{RX}}$ denotes the -3 dB antenna gain width. The constant $ D_{h,\epsilon} $ incorporates the satellite altitude and elevation angle, allowing the value $ D_{h,\epsilon}r $ to approximate the angle of an Earth transmitter in a satellite antenna pattern. A comprehensive explanation of this parameter is provided in Section \ref{sec:planarmodel}.
...
The GP is a \textit{projection process} mapping the points from $\mathbb{R}^2$ into $(0,\infty)$ and, as such, forms a nonhomogeneous PPP \cite{CITE}.
...
The parameter $\kappa$ in $\lambda_{\mathcal{G}}(t)$ can be interpreted as approximately the average number of Earth transmitters within the -3 dB footprint of a satellite BS (see Section \ref{sec:planarmodel}). It scales with the density parameter $\lambda$ of $\Phi$ with the relationship $\kappa \sim \lambda$.
...
Our findings indicate that employing IC significantly enhances the coverage probability. In a noise-limited channel with moderate noise, we demonstrate that the optimal value for the parameter $\tilde{\kappa} \upsilon_{}$ is approximately $\log(2)$, which maximizes the performance of the LEO uplink. Moreover, IC not only reduces the variance in both SIR and SINR (across a uniform distribution of satellite BSs) but also preserves overall performance. Thus, appropriate interference cancellation can improve user fairness in the network without compromising link quality.
