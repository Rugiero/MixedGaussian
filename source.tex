...
The joint probability density of the vector of $k$ strongest values of the STIR process $(\mathsf{Z}'_{(1)}, \dots, \mathsf{Z}'_{(n)})$ is given as a series expansion involving the partial densities
\begin{equation}
...
\end{equation}
with $\theta'=\theta/(1+\theta)$.


We still need to explicitly determine the expressions for the partial density ${\mu'}_n^{(n+i)}(\cdot)$ of the factorial moment measure $M^{(n)}(\cdot)$. We will tackle this separately for the interference-only channel and the interference-plus-noise-limited channel. For the interference-only channel, the results can be obtained from those of the interference-plus-noise-limited channel by setting the noise to $0$. However, the latter scenario is numerically more demanding.
...
\end{align}
if $\sum_{i=1}^k z_i + iz_k <0$ and $0$ otherwise. 
...
Let $\{u_1,\dots,u_k \}\subset \mathcal{G}$ be an ordered set of points in the GP, $u_1$ denoting the strongest signal at the SBS. Consider that the strongest $k$ signals are canceled from the interference. We define the SINR of the $n$\textit{th} strongest signal under interference cancellation (IC-SINR) as
\begin{equation}
  \text{SINR}_{n,k} \triangleq \frac{u_n}{N+I-\sum_{j =1 }^k u_j}.
\end{equation}


The IC-SINR coverage probability can be formulated as
...
For the interference-only channel, $N=0$, $\text{SIR}_{n,k} \triangleq \text{SINR}_{n,k}$
...
\begin{equation}
  \mathcal{P}^{(k)}_{\text{IIC}}(\theta,\tau) \triangleq \mathbb{P}\begin{Bmatrix} \text{SINR}_{1,k}>\theta & \text{if } \mathsf{Z}_{(k)}>\tau, \\
\mathsf{Z}_{(1)}>\theta & \text{otherwise} \end{Bmatrix}.
\end{equation}
Similarly, SIC-SINR$_{(k)}$ coverage probability is defined by
\begin{equation}
  \mathcal{P}^{(k)}_{\text{SIC}}(\theta,\tau) \triangleq \mathbb{P}\begin{Bmatrix} \text{SINR}_{1,k}>\theta & \text{if for all }n\in\{1,\dots,k\}  \\
    & \mathsf{Z}'_{(n)}+\tau'\sum\limits_{j=1}^{n-1}\mathsf{Z}'_{(j)}>\tau',\\
\mathsf{Z}_{(1)}>\theta & \text{otherwise} \end{Bmatrix}.
\end{equation}
Note that $\mathcal{P}^{(k)}_{\text{SIC}}(\theta,\tau)\geq \mathcal{P}^{(k)}_{\text{IIC}}(\theta,\tau) \geq \mathcal{P}^{(1,1)}(\theta)$ for all $0<\tau \leq \theta$. We observed that for $\tau = -7$ dB, the best results for SIC were obtained for $k=4$, whereas in IIC for $k=2$. (Of course, the optimal results would be obtained by not apriori restricting $k$, but always choosing the maximum amount of UEs meeting the cancellation criterion and to be removed from the interferers; however, we omit such a scheme in this work.) 
...
\begin{align}
  \Delta^{(k)}_{\text{IIC}}(\theta,\tau) \triangleq \mathcal{P}^{(k)}_{\text{IIC}}(\theta,\tau)- \mathcal{P}^{(1,1)}(\theta),\nonumber\\
  \Delta^{(k)}_{\text{SIC}}(\theta,\tau) \triangleq \mathcal{P}^{(k)}_{\text{SIC}}(\theta,\tau)- \mathcal{P}^{(1,1)}(\theta).
\end{align}
Using the joint probability density function of the STIR process \eqref{eq:jointprobability}, we have for the SIC-SINR
\begin{align}
  \label{eq:SICprob}
...
\end{align}
with the upper summation limit bounded by $i_{\text{max}} < 1/\tau'-1=1/\tau.$ Note the upper bound $\theta'$ of the integral, through which we omit the probability mass of the even $\mathsf{Z}_{(1)}>\theta$. The IIC case $\Delta^{(k)}_{\text{IIC}}(\theta,\tau)$ can be obtained by dropping the summation term inside the left-hand side identity function in $\eqref{eq:SICprob}$ (which only essentially lower bounds the integration domain by $\tau'$).
...
Combining \eqref{eq:SIRjointprobability}, \eqref{eq:jointprobability}, and \eqref{eq:factorialmoment}, we get a closed form for the pdf of SIR$_{1,1}$ in the region $z\geq 1$:
