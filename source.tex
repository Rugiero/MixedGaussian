...
\begin{table}
  \begin{center}
    \begin{tabular}{|c|p{4.5cm}|p{1.9cm}|}
      ...
      \multicolumn{3}{c}{\textbf{Principal symbols and parameter values in the numerical results}} \\
      \hline
      ...
      $\Phi\subset \R^2$ &Homogeneous PPP on planar Earth. & $\{(x_i,y_i)\}$\\
      $\Theta \subset \R^3 $ & Homogeneous PPP on spherical Earth. $[-\pi,\pi]\times[-1,1]\cap\Phi\ni(x_i,y_i)\mapsto(\rEarth.x_i,\arcsin y_i)\triangleq(\rEarth,\theta_i,\varphi_i)$, where $\rEarth$ is the mean radius of Earth. &$\{(\rEarth,\theta_i,\varphi_i)\}\break\subset\{6378\}$ km $\hfill \break\times[-\pi,\pi]$ rad $\break\times[-\frac{\pi}{2},\frac{\pi}{2}]$ rad. \\      
      ...
      $\kappa$  & $\text{Average (mean) number of }  \text{UEs inside}$ the satellite BS $-3 \text{ dB footprints};$ \hfill \break $\kappa =\kappa(\lambda,\epsilon) = \pi \lambda (h\varphi_{\textup{RX}}$ $/\sin^2(\epsilon))^2.$& \\
...
Let us consider that the $n$th strongest signal is decoded directly if its SINR exceeds $\theta$; otherwise, signal cancellation is attempted. We denote the SINR with independent interference cancellation as \text{IIC-SINR}$_{n,k}$, where interference from signals with indices in the set $[k] \triangleq \{1, \dots, k\}$ is canceled if each $(\mathsf{Z}_{(1)}, \dots  \mathsf{Z}_{(k)})$, reaches the signal separation threshold $\tau$, for which the sufficient condition is $\mathsf{Z}_{(k)}>\tau$. The coverage probability is defined by
