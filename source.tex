...
In this paper, we study the SIR under the SIC of user equipments (UEs) at a typical LEO satellite base station (LEO BS) in an urban shadowing environment. We utilize the Gaussian Mixture shadowing model, as proposed in \cite{modelinguplink} and \cite{9684552}. We demonstrate that, while maintaining good performance,  link quality can be made more stable with interference cancellation by increasing network density and mitigating interference with the SIC, enabling serving multiple UEs at each LEO BS.
...
In the paper, a mixture exponential distribution to approximate Gaussian Mixture shadowing is introduced, simplifying the analysis. %The exponential fading distribution typically yields the simplest and most insightful results in stochastic geometry analysis, which can be leveraged by the approximation.
...
Figures \ref{fig:nprobability} and \ref{fig:ASICSIR} depict the $k$-probabilities \eqref{eq:kprobability} and the SIC-SIR \eqref{eq:SICprob} for ${\kappa}\upsilon=\tilde{\kappa}\upsilon\log(2)=n=3$, respectively. The systems parameters are presented in Table \ref{table:parameters}. In the simulations, $\lambda$ is determined to match ${\kappa}\upsilon(\epsilon)=3$, for each $\epsilon \in \{30,90\}\degree$. The simulated coverage probabilities with elevation angles $\epsilon \in (30,90) \degree$ fall within the grayed areas in the plots.
