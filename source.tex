...
A narrowband LEO uplink is considered with narrow-beamed LEO receivers, and statistics over short use periods are studied. Due to the narrow beam, the Doppler spread can be ignored. The UEs follow a homogeneous PPP $\Phi \subset \R^2$ of density $\lambda$. No fast-fading is modeled, and the SBSs are associated with the served UEs according to their signal powers at the SBS after log-normal shadowing. The SBSs form a homogeneous point process (p.p.), possibly with a different density than the UEs (uniform constellation); two different SBSs might be partially associated with the same UEs. Because of the translation invariance of the PPP, all locations are statistically equivalent, and we define the origin $\textit{o}$ to represent the typical SBS footprint focus point.
