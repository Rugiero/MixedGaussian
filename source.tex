...
Considering the natural base of the log-normal distribution, the constant $\rho \triangleq \log(10)/10$ normalizes the parameters $\mu_{\text{LoS}}, \sigma_{\text{LoS}}, \mu_{\text{NLoS}},$ and $\sigma_{\text{NLoS}}$ so that $10 \log_{10}(H_{\mathcal{M}\mathcal{L} \mathcal{N}})$ evaluates to a mixed normal r.v. $p_{\text{LoS}} \mathcal{N}(\mu_{\text{LoS}}, \sigma_{\text{LoS}}^2) + p_{\text{NLoS}} \mathcal{N}(\mu_{\text{NLoS}}, \sigma^2_{\text{NLoS}})$.
 ...
 Furthermore, we introduce a constant scaling term, $\Upsilon_{\epsilon}$, ensuring that the means of the mixed log-normal distribution and the defected exponential distributions match; $\mathbb{E}(\Upsilon^{}_{\epsilon}H_{\mathcal{M}\mathcal{L}\mathcal{N}})= \mathbb{E}(H_{\text{Exp}}) = \upsilon_{\epsilon}$. The scaling is accounted for by adjusting the noise constant by a factor of $N \sim 1/\Upsilon_{\epsilon}$. Namely, by equating the first two moments of $H_{\text{Exp}}$ (l.h.s.) and $\Upsilon_{\epsilon} H_{\mathcal{M} \mathcal{L}\mathcal{N}}$ (r.h.s.),
\begin{equation}
...
\end{align}
Recall equations \eqref{eq:totmean} and \eqref{eq:totvar}. Matching the first first two moments of the fading distributions $H=H_{\text{Exp}}$ and $H=H_{\mathcal{M}\mathcal{L}\mathcal{G}}$ is equivalent to matching the mean and variance of the corresponding total interferences (before the spatial path loss) $I = \sum_{x \in \mathcal{G}} H_x x$.


 Without loss of generality, we can set $\Upsilon_{\epsilon}=1$  when studying the SIR scenarios, as the scaling of the signal powers cancel for $N=0$.
