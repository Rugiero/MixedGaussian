...
Without loss of generality, we can set $\Upsilon_{\epsilon}=1$  when studying the SIR scenarios, as the scaling of the signal powers cancel for $N=0$. In the SINR case, the scaling is accounted for by adjusting the noise constant by a factor of $N \sim 1/\Upsilon_{\epsilon}$. The parameter $0 \leq  1-\upsilon_{\epsilon} < 1$ of the defective exponential distribution encapsulates the shadowing occurrences, when the signal is either blocked by an obstacle or destructively interfered by its multi path components mapping the signal power very close to $-\infty$ dB in the mixed log-normal shadowing model. These occurrences are particularly common in low elevation-angles, because of tall buildings and other similar large objects preventing LoS and hence causing severe signal blockage at the SBS. Note that the signals are not blocked identically to all directions, hence the signal strengths and the order statistics of the SINR and STINR processes depend on the serving SBS location w.r.t. the UEs. (Let us, for example, consider that there is a large building between the SBS$_1$ and UE$_1$. The signal strength of the UE$_1$ may well map to $0$ at SBS$_1$. On the other hand the strongest signal at the SBS$_1$ could be from another UE$_2$. However, on the other side of the building, UE$_1$ might well be the strongest signal at a SBS$_1$ located in the other side of the building, whereas UE$_2$ would cause (?!?!?!) unsignificant interference. In other words, SBS$_1$ and SBS$_2$ would serve different UEs with good performance, even though the distances of the UEs and SBS would be identical.
