...
\begin{prop}[Density of the GP]
  ...
  \end{equation}
  where $D_{h,\epsilon}=\sin^2(\epsilon)/h$. The parameter $\kappa$ has an interpretation as the mean number of UEs inside the $3$ dB footprint of a SBS (see appendix TÄYTÄ).
  \begin{proof}
...
\end{equation}
of normalized jumps of the subordinator. Directly from the definition, it is the STIR process $(\mathsf{Z}'_{(1)},\mathsf{Z}'_{(2)} \dots)$. It has the Poisson-Dirichlet distribution PD$(0, \upsilon_{\epsilon} \tilde{\kappa})$. 
...
The mean number of UE inside the $3$ dB footprint $\times \log(2)$ multiplied the portion of the non-zero signals; $ \tilde{\kappa} \upsilon_{\epsilon}$, appears in the Laplace transform of $I$ \eqref{eq:lapdef}, the $n$th factorial moment measures \eqref{eq:factorialmoment} and \eqref{eq:nthmomentmeasureSTINR} of the SINR and STINR processes. Recall \eqref{eq:GPdensity}: regardless of $h, \lambda, \text{ or } \varphi_{\text{RX}}$, the essential parameter is the the mean number of the \textit{effective} UEs inside the $3$ dB footprint of a SBS.
