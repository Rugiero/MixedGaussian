...
\maketitle
         ...
         \end{table}   
          ...
          Let $W>0$ be a normalized noise power constant. We consider a NB and define the spatial path-loss to be equal to all transmitters, and depending only on the distance between the SBS and its antennas boresights location on the earth's surface $d_{h,\epsilon}$. The path-loss law is defined as $\ell({d}) \triangleq ({d}/d_0)^{\gamma}$, where $\gamma$ is the power path-loss exponent. The SINR of the strongest signal $x_0 \triangleq \max  \mathcal{G}$ at the typical SBS is defined as
          \begin{equation}
            \text{SINR} = \frac{x_0/\ell(\hat{d}_{h,\epsilon})}{\sum_{x \in \mathcal{G} \setminus \{x_0\}}x/\ell(\hat{d}_{h,\epsilon})+ W}.
          \end{equation}
          ...
          \begin{prop}
            Assume a network of density $\lambda$ with the tiers $\mathcal{T}$. Let $H_{x_0}$ be the shadowing variable, and ${x'}_0 =  H_{x_0}G(\|x_0\|) \in \cup_{i \in \mathcal{T}} \mathcal{G}_i$ be the strongest signal of the UEs. In the simple coverage region $\theta \geq 1$,
            \begin{align}
              \label{eq:probofcov}
              &p_c(\theta) \triangleq \mathbb{P}\left( \frac{H_{x_0} G(\|x_0\|)}{\sum_{x' \in  \cup\mathcal{G}_i \setminus \{x'_0\}}x'  + W\ell(\hat{d}_{h,\epsilon})} > \theta \right) \nonumber \\      
              ...
              &\overset{(c)}{=}  2 {} \sum_{i \in \mathcal{T}}\kappa_i\int_0^{\infty}u  \frac{\exp \{- \theta 2^{u^2}W\ell(\hat{d}_{h,\epsilon})/m_i \}}{\prod_{k \in \mathcal{T}}\left(1 + \theta2^{u^2}  m_k/m_i\right)^{\tilde{\kappa}_k}} du,
            \end{align}
            ...
            \begin{proof}
              ...
              In (b), an exponential $(H_{x_0}|T_{x_0}=i)$ allows the expression of the probability in terms of the Laplace transforms of $\sum_{x \in \cup \mathcal{G}_i}x$ and $W$. The formers are gamma distributed. Also, by the independent shadowing, the are independent and can be expressed as the product. 
            \end{proof}
          \end{prop}
