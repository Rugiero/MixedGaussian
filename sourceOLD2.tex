...
We conclude that SIC is a viable solution to reduce the considerable variability in link quality from the strongest transmitters over the satellite base stations (SBSs), while maintaining the average performance. In addition, the SIR of the second and third strongest transmitters at each SBS is significantly improved. This means a single SBS can serve multiple UEs if the SIC is implemented.

This work proposes a tractable analytical framework for the SIC and its effect on the performance of the narrow-beam LEO network.
