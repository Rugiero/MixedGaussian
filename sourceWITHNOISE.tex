...
p_c(\theta) =  \theta^{-\tilde{\kappa}b} {_2F_1}\left(\tilde{\kappa}b, \tilde{\kappa}b;\tilde{\kappa}b + 1; -1/\theta \right),
          ...
          \begin{prop}
            Let $\mu_M \triangleq \sum_{i=1}^nt'_i/G(D_{h,\epsilon}\|x_i\|)$ and ${T}'_n\triangleq T_n(t_1,\cdots,t_n)=1/(1-\sum_{i=1}^nt'_i)$. The $n$-moment measure of the STINR process is given by
            \begin{align}
            ...
            \begin{align}
              &M'^{(n)}(t'_1,\dots,t'_n) =\left(\frac{\tilde{\kappa} b}{\lambda}\right)^n \nonumber\\
              & \times\int\limits_{(1,\infty)^n} \exp\left\{-aW\ell(\hat{d}_{h,\epsilon})T_n\sum\limits_{i=1}^nt'_iv_i\right\} \left(1+T_n\sum\limits_{i=1}^nt'_iv_i\right)^{-\tilde{\kappa}b}  \nonumber\\
              &\hspace{0.8cm}\times \frac{\prod\limits_{i=1}^nt'_i}{\sum\limits_{i=1}^nt'_iv_i}  \left[\sum\limits_{j=1}^n\frac{1}{\prod\limits_{i\neq j}\left(t'_i v_i+a t_i'T_n\sum\limits_{k=1}^nt'_kv_k \right)} \right] \prod_{i=1}^n dv_i.
              \end{align}
          \end{prop}
